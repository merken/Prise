...
namespace LegacyPlugin1_4
{
    [Plugin(PluginType = typeof(ITranslationPlugin))]
    public class FromFrenchTranslationPlugin : TranslationPluginBase
    {
        internal FromFrenchTranslationPlugin(IConfiguration configuration,  IDictionaryService dictionaryService)
            :base(configuration, dictionaryService)
        {
        }

        [PluginFactory]
        public static FromFrenchTranslationPlugin ThisIsTheFactoryMethod(IServiceProvider pluginServices)
        {
            ...
            return new FromFrenchTranslationPlugin(
        ...
        protected override string GetLanguageCode() => "fr-FR";
    }
}
